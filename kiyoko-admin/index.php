...
<div class="animate__animated p-6" :class="[$store.app.animation]">
    ...
    <!-------------------------------- seting snow  -->
    <div class="m-3 panel space-y-5 w-50" style="width: auto;min-width: 350px;">
        <h5 class="mb-4 text-lg font-semibold">Setting</h5>
        <form method="POST" action="index.php">
        <div class="d-flex align-items-center" style=" display: flex; align-items: center;gap:7px">
            <p class="mb-0 mr-3">Enable Snow Effect: <span class="text-primary"><?php echo ($snow_effect == 1) ? 'Enabled' : 'Disabled'; ?></span></p>
            <label class="relative h-6 w-12" class="d-block">
                ...
                <?php echo ($snow_effect == 1) ? 'checked' : ''; ?> name="snow_effect">
        ...
        <div class="div" style=" display: flex; align-items: center;gap:7px">
            <p style="height: 25px;">Store <span class="text-primary">status</span> (coming soon / open)</p>
                <select class="form-control" id="store_status" name="store_status" class="d-block" style="box-shadow: 0 0 5px rgb(243,243,243);height: 25px;padding: 5px;">
                    <option value="open" <?php echo ($store_status == 'open') ? 'selected' : ''; ?>>Open</option>
                    <option value="coming_soon" <?php echo ($store_status == 'coming_soon') ? 'selected' : ''; ?>>Coming Soon</option>
                </select>
            
        </div>
        <button type="submit" name="update_settings" class="btn btn-primary btn-sm mt-3">Update Settings</button>
        </form>
    </div>

    <?php
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update_settings'])) {
            $new_snow_effect = isset($_POST['snow_effect']) ? 1 : 0;  
            $new_store_status = $_POST['store_status'] ?? 'open';   
            $update_query = "UPDATE settings SET snow_effect = ?, store_status = ? WHERE id = 1";
            $stmt = $conn->prepare($update_query);
            $stmt->bind_param("is", $new_snow_effect, $new_store_status);
    ...
    <?php 
        ...
        ?>
    <?php 
        $queryslide = "SELECT * FROM slider_images";
        $resultslide = mysqli_query($conn, $queryslide);
    ?>
    <div class="panel h-full w-full mt-5z mt-5">
        <div class="mb-5 flex items-center justify-between">
            <h5 class="text-lg font-semibold dark:text-white-light">Slider Image Management</h5>
        ...
        <div class="table-responsive">
            <table class="table table-bordered">
                ...
                <tbody>
                    <?php while ($rowslide = mysqli_fetch_assoc($resultslide)) { ?>
                        <tr>
                            <td><?php echo $rowslide['id']; ?></td>
                            <td><img src="../main/photos/<?php echo $rowslide['image_path']; ?>" alt="image" width="200" style="border-radius: 7px;"></td>
                            <td><?php echo $rowslide['title']; ?></td>
                            <td><?php echo $rowslide['subtitle']; ?></td>
                            <td><a href="<?php echo $rowslide['link']; ?>" target="_blank"><?php echo $rowslide['link']; ?> </a></td>
                            <td style="display: flex; gap:6px;align-items: center;height: 100%;">
                                <a href="edit_image.php?id=<?php echo $rowslide['id']; ?>" class="btn btn-warning btn-sm">Edit</a>
                                <a href="javascript:void(0);" onclick="confirmDelete(<?php echo $rowslide['id']; ?>)" class="btn btn-danger btn-sm">Delete</a>
                            </td>
                        </tr>
                    <?php } ?>
                </tbody>
    ...
    function confirmDelete(imageId) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                // Nếu người dùng xác nhận, chuyển hướng tới trang xóa ảnh
